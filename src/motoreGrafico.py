...
def generateMaze(WIDTH, HEIGHT, grid, w):
    maze = []
    for cell in grid:
        x = cell.i*w
        y = cell.j*w
        if cell.walls[0]:
            maze.append(Line(x, y, x+w, y))
        if cell.walls[1]:
            maze.append(Line(x+w, y, x+w, y+w))
        if cell.walls[2]:
            maze.append(Line(x+w, y+w, x, y+w))
        if cell.walls[3]:
            maze.append(Line(x, y+w, x, y))
    return maze
...
def drawRays(obstacles, screen, nRays, xPlayer, yPlayer, delta, direction, r, distances, deltaX, WIDTH, HEIGHT, w, mode):
    ...
    if mode:
        ...
        for i in range(nRays):
            x       = deltaX*i
            # x       = deltaX*i + WIDTH/2
            h       = (r-(distances[i])*cos(delta*(i-nRays/2)))
            surf    = pygame.Surface((deltaX, h))
...
def checkWall(maze, i, j, newi, newj, cols):
    from src.functions import index
    x = i-newi
    if (maze[index(i, j, cols)]).walls[3] and x == 1:
        return True
    if (maze[index(i, j, cols)]).walls[1] and x == -1:
        return True
    y = j-newj
    if (maze[index(i, j, cols)]).walls[0] and y == 1:
        return True
    if (maze[index(i, j, cols)]).walls[2] and y == -1:
        return True

# Main
def game(grid, cols, rows, w, mode):
    import pygame
    ...
    maze        = generateMaze(WIDTH, HEIGHT, grid, w)
    distances   = [r for i in range(nRays)]
    fov         = pi/4
    ...
    while running:
        ...
        delta   = fov/nRays
        screen.fill(0)
        (i, j)      = position(xPlayer, yPlayer, w)
        xPlayer    += movementY*cos(direction)*velocity
        yPlayer    += movementY*sin(direction)*velocity
        direction  += movementX*0.1
        (newi, newj)= position(xPlayer, yPlayer, w)
        if (i,j) != (newi, newj):
            if checkWall(grid, i, j, newi, newj, cols):
                xPlayer -= movementY*cos(direction)*velocity
                yPlayer -= movementY*sin(direction)*velocity


        # First screen
        drawRays(maze, screen, nRays, xPlayer, yPlayer, delta, direction, r, distances, deltaX, WIDTH, HEIGHT, w, mode)
        pygame.display.flip()
